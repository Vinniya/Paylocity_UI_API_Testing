...
public class EmployeePage {
	...
	/*
	 * Add an employee by entering the firstname, lastname and dependents 
	 * Date:
	 * Owner: Vinniya
	 */
	 ...
	 * Make sure added employee details exists in employees table
	 * Date: 
	 * Owner: Vinniya
	 */
	public void verifyEmployeeTable() {
		...
		for (int i = 1; i <= rows.size(); i++) {
			...
			for (int j = 1; j <= columns.size(); j++) {
				...
				if (columns.get(j + 1).getText() == firstname && columns.get(j + 2).getText() == lastName
						&& columns.get(j + 3).getText() == dependents && columns.get(j + 4).getText() == salary
						&& columns.get(j + 5).getText() == gross_pay && columns.get(j + 6).getText() == benefits_cost
	...
	public void editEmployee() {
		...
		for (int i = 1; i <= rows.size(); i++) {
			...
			for (int j = 1; j <= columns.size(); j++) {
				...
				if (columns.get(j + 1).getText() == "vinniya" && columns.get(j + 2).getText() == "velusamy"
						&& columns.get(j + 3).getText() == "2") {
					WebElement edit = driver
							.findElement(By.xpath("//table/tbody/tr[" + i + "]/td/i[@class='fas fa-edit']"));
	...
	public void deleteEmployee() {
		...
		for (int i = 1; i <= rows.size(); i++) {
			...
			for (int j = 1; j <= columns.size(); j++) {
				...
				if (columns.get(j + 1).getText() == "John" && columns.get(j + 2).getText() == "Martin"
						&& columns.get(j + 3).getText() == "2") {
					WebElement delete = driver
							.findElement(By.xpath("//table/tbody/tr[" + i + "]/td/i[@class='fas fa-times']"));
